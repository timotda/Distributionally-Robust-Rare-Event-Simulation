...
import numpy as np
import pandas as pd 
from get_worst_p_linear_cis_mc import get_worst_p_linear_cis_mc
...
def compute_average_worst_prob(delta, loss_threshold, mu, Sigma, w, N_alpha,N_tot, runs, rng):
    """
    ...
    u_bar,_,_,var_H_,var_p_= get_worst_p_linear_cis_mc(delta,loss_threshold,mu,Sigma,w,1/2,N_alpha,rng)
    ...
    for i in range(runs):
        ...
        u_bar_,p_hat, _, var_H,var_p= get_worst_p_linear_cis_mc(delta,
                                            loss_threshold,
                                            mu,
                                            Sigma,
                                            w,
                                            ...
                                            N_tot-N_alpha,
                                            rng,u_opt=1.1*u_bar) #### change u_opt if can't find different fmin and fmax
...
def run_threshold_sweep(
    thresholds, delta, mu, Sigma, w, N_alpha, N_tot, runs, rng, out_csv="results.csv"
):
    results = []
...
if __name__ == "__main__":
    ...
    run_threshold_sweep(
        ...
        runs=runs,
        rng=rng,
        out_csv="CIS_MC_linear_results using resampled.csv"
    )
    '''
