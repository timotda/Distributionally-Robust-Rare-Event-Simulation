...
def compute_average_worst_prob(delta, loss_threshold, mu, Sigma, w, N_alpha,N_tot, runs, rng):
    ...
    u_bar,_,_,var_H,var_p = get_worst_p_linear_mc_mc(delta,loss_threshold,mu,Sigma,w,1/2,N_alpha,rng)
    alpha = np.sqrt(var_H)/(np.sqrt(var_H)+ u_bar*np.sqrt(var_p))
    if alpha == 1.0:
        alpha = 1/2
    if np.isnan(alpha):
        alpha = 1/2
    ...
    for i in range(runs):
        ...
        u_bar_,p_hat, _, var_H,var_p = get_worst_p_linear_mc_mc(delta,
                                            loss_threshold,
                                            mu,
                                            Sigma,
                                            w,
    ...
    return mean_p, ci_half, mean_time,var_H,var_p,u_bar,lsre_mean, lsre_median, lsre_arr
