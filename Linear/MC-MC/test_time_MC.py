...
# ---------- Timed wrapper ----------
def timed_get_worst_p_linear_mc_mc(delta, loss_threshold, mu, Sigma, w, alpha, N, rng):
    """Run MC and record wall-clock time of each logical block."""
    t = defaultdict(float)
    ...
    t["get_radius_mc"] += time.perf_counter() - tic

    # --- Stage 2 ---
    tic = time.perf_counter()
    p_hat, err, var_p = robust_mc_linear(u_bar,A,b0,N2,rng)
    t["robust_mc_linear"] += time.perf_counter() - tic
...
def bench(runs, **kwargs):
    ...
    last_outputs = None
    for _ in range(runs):
        outputs = timed_get_worst_p_linear_mc_mc(**kwargs)
        *vals, tdict = outputs
        last_outputs = vals
