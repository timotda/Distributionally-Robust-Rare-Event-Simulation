...
import numpy as np
import pandas as pd 
from get_worst_p_linear_mc_cis import get_worst_p_linear_mc_cis
...
def compute_average_worst_prob(delta, loss_threshold, mu, Sigma, w, N_alpha,N_tot, runs, rng):
    ...
    u_bar,_,_,var_H_,var_p_= get_worst_p_linear_mc_cis(delta,loss_threshold,mu,Sigma,w,1/2,N_alpha,rng)
    ...
    for i in range(runs):
        ...
        u_bar_,p_hat, _, var_H,var_p= get_worst_p_linear_mc_cis(delta,
                                            loss_threshold,
                                            mu,
                                            Sigma,
                                            w,
...
if __name__ == "__main__":
    ...
    loss_threshold = 1        #  loss‐threshold
    ...
    run_threshold_sweep(
        ...
        runs=runs,
        rng=rng,
        out_csv="MC_CIS_linear_results.csv"
    )
    '''
