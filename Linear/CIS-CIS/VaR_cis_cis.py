...
def get_VaR_linear_cis_cis(beta, delta, mu, Sigma, w, N,rng):
    """
    ...
    # Define f(u) = p̂(u) - β
    def f(u):
        _, p_hat, _ ,_,_= get_worst_p_linear_cis_cis(delta, u, mu, Sigma, w,1/2, N,rng)
        return p_hat - beta
...
def bootstrap_VaR_linear_cis_cis(beta, delta, mu, Sigma, w, N, rng,s=30):
    """Bootstrap the above estimator to get a 95%-CI half-width."""
    var_estimates = [
        get_VaR_linear_cis_cis(beta, delta, mu, Sigma, w, N,rng)
