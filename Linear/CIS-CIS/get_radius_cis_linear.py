...
def get_radius_cis(d2, L, delta2):
    """
    Solve for u in ℎ̂(u) ≡ (1/N) ∑ d2_i 1{d2_i ≤ u} L_i = δ².

    Parameters
    ----------
    d2 : ndarray, shape (N,)
        Squared distances from the CIS sampler.
    L : ndarray, shape (N,)
        Likelihood ratios from the CIS sampler.
    delta2 : float
        Target δ².
    bracket : tuple (umin, umax), optional
        Search interval. Defaults to [0, max(d2)].
