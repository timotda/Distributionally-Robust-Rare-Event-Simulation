...
import numpy as np
import pandas as pd
from get_worst_p_piecewise_cis_cis import get_worst_p_piecewise_cis_cis
...
def piecewise_active_report(A, B, a, b, w, v, x0=None, tol=1e-8, verbose=True):
    """
    Find the closest point x* to the piecewise set { w^T max(Ax+a, Bx+b) <= -v }
    and report which branches (A or B) are active at x*, plus active rank in x-space.

    Returns a dict with:
      - x_star, R
      - y_star
      - idx_A, idx_B (indices active on A- or B-branch; ties appear in both)
      - active_size, rank_active
      - check_wty (value of w^T y_star, should be ~ -v at the boundary)
    """
    d = A.shape[1]
    if x0 is None:
        x0 = np.zeros(d)

    # 1) closest point to the set
    # NOTE: this solver returns (x_star, distance) when return_x_star=True
    x_star, R = solve_distance(
        x0, A, B, a, b, w, v,
        return_x_star=True
    )

    # 2) compute the piecewise y* = max(Ax*+a, Bx*+b)
    Ax = A @ x_star + a
    Bx = B @ x_star + b
    y_star = np.maximum(Ax, Bx)

    # 3) determine which branch is active per component (tight within tolerance)
    # scale the tolerance to the magnitude of y*
    scale = max(1.0, norm(y_star), norm(Ax), norm(Bx))
    thr = tol * scale

    slack_A = y_star - Ax
    slack_B = y_star - Bx

    idx_A = np.where(slack_A <= thr)[0]  # active on A-branch
    idx_B = np.where(slack_B <= thr)[0]  # active on B-branch

    # 4) build active normal matrix in x-space and get its rank
    G_act = []
    if idx_A.size:
        G_act.append(A[idx_A, :])
    if idx_B.size:
        G_act.append(B[idx_B, :])
    if G_act:
        G_act = np.vstack(G_act)
        rank_act = matrix_rank(G_act)
        active_size = G_act.shape[0]
    else:
        rank_act = 0
        active_size = 0
    ...
    if verbose:
        print(f"R = {R:.6g}")
        print(f"Active A-branch count: {len(idx_A)}")
        print(f"Active B-branch count: {len(idx_B)}")
        print(f"Active size (rows stacked): {active_size}   rank: {rank_act}")

        
    return dict(
        x_star=x_star, R=float(R), y_star=y_star,
        idx_A=idx_A, idx_B=idx_B,
        active_size=int(active_size), rank_active=int(rank_act)
    )


def compute_average_worst_prob(delta, loss_threshold, mu, Sigma, w, N_alpha,N_tot, runs, rng,A,B,a,b):
    """
    ...
    u_bar,_,_,var_H_,var_p_= get_worst_p_piecewise_cis_cis(delta,loss_threshold,mu,Sigma,w,1/2,N_alpha,rng,A,B,a,b)
    ...
    for i in range(runs):
        ...
        u_bar_,p_hat, _, var_H,var_p= get_worst_p_piecewise_cis_cis(delta,
                                            loss_threshold,
                                            mu,
                                            Sigma,
                                            w,
...
if __name__ == "__main__":
    ...
    loss_threshold = 8      #  loss-threshold
    N_tot = int(1e5)          # number of MC samples per run
    N_alpha = int(10000)      # number of samples used to find the optimal allocation
    runs = 5            # number of independent repetitions
    # ==============================

    # (stock*(1+return)-stock)/stock
    # (stock*(1+return)-strike-option)/option
    w = np.ones((2 * n, 1), dtype=float) / (2 * n)

    A_tmp = np.empty(n, dtype=float)
    a_tmp = np.empty(n, dtype=float)

    A_tmp[is_call] =  stock_price[is_call] / option_price[is_call]
    a_tmp[is_call] = -1.0 + (stock_price[is_call] - strike_price[is_call]) / option_price[is_call]

    A_tmp[~is_call] = -stock_price[~is_call] / option_price[~is_call]
    a_tmp[~is_call] = -1.0 + (strike_price[~is_call] - stock_price[~is_call]) / option_price[~is_call]
