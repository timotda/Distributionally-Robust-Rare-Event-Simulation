...
import numpy as np
import pandas as pd
from get_worst_p_piecewise_mc_cis import get_worst_p_piecewise_mc_cis

def lsre_from_plug_in(p_hat, u_hat, var_H, var_P, alpha):
    num = (var_H / (alpha * (u_hat**2))) + (var_P / (1.0 - alpha))
    return np.sqrt(num) / p_hat
def rel_hw_from_lsre(lsre, N, runs=1, z=1.96):
    lsre = np.asarray(lsre, dtype=float)
    rel_hw_single = z * lsre / np.sqrt(N)
    rel_hw_mean   = rel_hw_single / np.sqrt(runs)
    return rel_hw_single, rel_hw_mean
...
def compute_average_worst_prob(delta, loss_threshold, mu, Sigma, w, N_alpha,N_tot, runs, rng,A,B,a,b):
    ...
    u_bar,_,_,var_H_,var_p_= get_worst_p_piecewise_mc_cis(delta,loss_threshold,mu,Sigma,w,1/2,N_alpha,rng,A,B,a,b)
    ...
    for i in range(runs):
        ...
        u_bar_,p_hat, _, var_H,var_p= get_worst_p_piecewise_mc_cis(delta,
                                            loss_threshold,
                                            mu,
                                            Sigma,
                                            w,
...
def run_threshold_sweep(
    ...
    for loss_threshold in thresholds:
        ...
        # you can also compute single‐run relative half‐width if you like:
        ...
        results.append({
            ...
            "rel_hw_single(%)":  rel_single.mean()*100,
            "rel_hw_mean(%)":    rel_mean.mean()*100,
        })
...
if __name__ == "__main__":
    ...
    '''
    loss_thresholds = [1, 2,8,10] #, 4, 6, 10]
    ...
    run_threshold_sweep(
        ...
        out_csv="MC_CIS_piecewise_results.csv"
    )
