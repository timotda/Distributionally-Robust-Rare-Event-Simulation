...
def get_worst_p_no_robust(
    ...
    X, L, _ = shift_samples(G, h, R, QM, 0.0, N_total, rng)  # u=0

    p_hat = np.mean((G @ X >= h[:,None]).all(axis=0) * L)
    re = np.sqrt(np.var(((G @ X >= h[:,None]).all(axis=0) * L), ddof=1)) / max(p_hat, 1e-300)
    return p_hat, re
